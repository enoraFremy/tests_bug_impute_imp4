...
GetListFuncs <- function(obj=NULL){
  ...
  if (is.null(obj)) {
    ...
    ll <- c(#"wrapper.dapar.impute.mi", #peptide
           # "wrapper.dapar.impute.mi", #peptide
            "wrapper.impute.mle", #peptide
            #"wrapper.impute.slsa",
    ...
    ll <- #list(obj,nb.iter = 1, lapala = FALSE, progress.bar = FALSE),
               #list(obj,nb.iter = 1, lapala = TRUE, progress.bar = FALSE),
               list(obj)
               #list(obj),
